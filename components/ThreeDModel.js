...
const ThreeDModel = () => {
  ...
  const [initialCameraPosition] = useState(
    new THREE.Vector3(
      20 * Math.sin(0.1 * Math.PI) * 2,
      10,
      10 * Math.cos(0.1 * Math.PI) * 2
    )
  )
    ...
    if (container && !renderer) {
      ...
      const animate = () => {
        ...
        if (frame <= 100) {
          ...
          camera.position.y = 10
          camera.position.x =
            -1 * (p.x * Math.cos(rotationSpeed) + p.z * Math.sin(rotationSpeed))
          camera.position.z =
            -1 * (p.z * Math.cos(rotationSpeed) - p.x * Math.sin(rotationSpeed))
          camera.lookAt(target)
        } else {
          controls.update()
        }
